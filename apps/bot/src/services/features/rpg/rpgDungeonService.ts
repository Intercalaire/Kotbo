...
export async function listGuildDungeons(guildId: string) {
  const [dungeons, bosses, runs] = await Promise.all([
    prisma.rpgDungeon.findMany({ where: { guildId }, orderBy: [{ levelRequired: 'asc' }, { name: 'asc' }] }),
...
export async function saveGuildDungeon(
  ...
  if (data.completionItemName && !(await findGuildItem(prisma, guildId, data.completionItemName))) {
    throw new DungeonError(`L'objet « ${data.completionItemName} » n'existe pas dans le catalogue.`, 400);
  }
...
export async function getDungeonHall(guildId: string, userId: string): Promise<DungeonHallEntry[]> {
  const [dungeons, bosses, lastRuns, clears] = await Promise.all([
    prisma.rpgDungeon.findMany({ where: { guildId, enabled: true }, orderBy: [{ levelRequired: 'asc' }, { name: 'asc' }] }),
    loadGuildBosses(guildId),
...
export async function leaveDungeon(guildId: string, userId: string, runId: string): Promise<DungeonSettlement> {
  ...
  return { outcome: 'LEFT', ...settled, levelUp };
}
...
export async function fightDungeonFloor(
  ...
  if (outcome && paid) {
    const levelUp = paid.xp > 0 ? await levelUpAfter(guildId, userId, profile.level) : null;
    settlement = { outcome, dungeon: run.dungeon, ...paid, levelUp };
  }

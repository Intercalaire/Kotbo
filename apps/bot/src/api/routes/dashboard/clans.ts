...
export async function handleClansRoutes(
  ...
  if (subAction === 'rebalance' && method === 'POST') {
    ...
    try {
      ...
      const isInputError = err instanceof RebalanceInputError || String(err?.message ?? '').includes('en cours');
      json(res, isInputError ? 400 : 500, { error: err?.message ?? 'Erreur lors du rééquilibrage.' });
    }
    return true;
  }

<script lang="ts">
  ...
  onMount(async () => {
    ...
    try {
      ...
      const [fRes, rRes] = await Promise.all([
        dashboardFetch(`/recruitment/forms/${formId}`, { headers: requestHeaders }),
        dashboardFetch(`/recruitment/forms/${formId}/responses?limit=100`, { headers: requestHeaders }),
      ]);
      if (fRes.ok) {
  ...
  async function loadMore() {
    ...
    loadingMore = true;
    try {
      const res = await dashboardFetch(`/recruitment/forms/${formId}/responses?limit=100&offset=${responses.length}`);

      ...
      <div class="flex items-center gap-4">
        {#if authStore.user}
          <div class="flex items-center gap-3 bg-white/5 border border-white/10 px-4 py-2 rounded-lg">
            ...
            <button onclick={logout} class="text-2xs font-semibold text-rose-500 uppercase tracking-widest hover:underline">{m.ctv_disconnect()}</button>
          </div>
        {/if}
      </div>
    </nav>
      ...
      <div class="relative group">
        ...
        <div class="relative bg-slate-900/40 rounded-xl border border-white/10 p-10 md:p-14 shadow-sm overflow-hidden">
          ...
          <div class="flex flex-col items-center text-center mb-10">
            ...
            <div class="space-y-4">
              <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/20">
                 ...
                 <span class="text-2xs font-semibold uppercase tracking-wider text-amber-500">{m.ctv_security_required()}</span>
              </div>
          ...
          <div class="space-y-6">
            ...
            <div class="space-y-2">
              <label for="activation-code" class="text-2xs font-semibold text-slate-500 uppercase tracking-widest block ml-1">{m.ctv_global_activation_key()}</label>
              <div class="relative">
            ...
            >
              ...
              <div class="relative flex items-center justify-center gap-4 py-5 px-8 text-on-primary font-semibold uppercase tracking-widest text-2xs transition-transform active:scale-95">
                {#if loading}
            ...
            {#if authStore.guilds.length > 1}
              <div class="pt-6 border-t border-white/5 space-y-3">
                <label for="guild-select" class="text-2xs font-semibold text-slate-500 uppercase tracking-widest block text-center">{m.ctv_access_another_server()}</label>
                <div class="relative">
                  <select 
                    id="guild-select"

<script lang="ts">
  ...
  import { NODE_CATALOG, PORT_COLORS, canConnect, type NodeDef, type PortDataType } from '@kotbo/shared';
  ...
  /** Premier port du bloc capable d'accueillir - ou d'alimenter - le fil tiré. */
  function matchingPort(def: NodeDef): string | null {
    const ports = direction === 'source' ? def.inputs : def.outputs;
    const fits = ports.find((port) => (direction === 'source'
  ...
  const candidates = $derived(
    NODE_CATALOG
      ...
      .map((def) => ({ def, portId: matchingPort(def) }))
      .filter((entry) => entry.portId !== null)
      .filter((entry) => (
        !search.trim()
...
<div class="fixed inset-0 z-50 flex items-start justify-center p-4 pt-24 bg-black/50 backdrop-blur-sm" role="presentation" onclick={onClose}>
  ...
  >
    ...
    <div class="p-3 space-y-1.5">
      ...
      {:else}
        {#each candidates as entry (entry.def.type)}
          <button
            type="button"
            onclick={() => onPick(entry.def.type, entry.portId as string)}
            class="w-full flex items-start gap-3 px-3 py-2.5 rounded-xl text-left bg-surface-container-highest/50 border border-outline-variant/10 hover:border-primary/40 hover:bg-surface-container-highest transition-all"
          >

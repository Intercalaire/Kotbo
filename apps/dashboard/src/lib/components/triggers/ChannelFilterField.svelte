...
<div class="space-y-1.5 {compact ? 'nodrag' : ''}">
  {#if !compact}
    <p class="text-[11px] font-medium text-on-surface-variant/80">{m.wf_channel_filter_label()}</p>
  {/if}
  ...
  <div class="flex flex-wrap gap-1.5">
    ...
    {:else}
      <span class="text-[11px] text-on-surface-variant/70">{m.wf_channel_filter_all()}</span>
    {/each}
  </div>

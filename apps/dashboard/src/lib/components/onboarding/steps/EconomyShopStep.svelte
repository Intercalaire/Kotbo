      ...
      <div>
        ...
        {#if picked}
          ...
          <div class="editor mt-1.5 ml-9 flex items-center gap-2 rounded-xl border border-outline-variant/25 bg-surface-container-lowest/40 px-3 py-2">
            <label for="price-{item.key}" class="text-xs text-on-surface-variant/55">
            ...
            <input
              ...
              class="w-24 rounded-lg border border-outline-variant/40 bg-surface-container-lowest/60 px-2 py-1
                     text-xs tabular-nums text-on-surface focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/40"
            />
            <span class="text-xs text-on-surface-variant/50 truncate">{emoji} {currency}</span>
            <span class="ml-auto shrink-0 text-2xs text-on-surface-variant/40">{effectOf(item)}</span>
          </div>
        {/if}
      </div>
    {/each}
  ...
  {#snippet preview()}
    <div class="rounded-xl overflow-hidden border border-black/25 shadow-sm">
      <div class="flex items-center gap-2 px-3.5 py-2.5 bg-[#2b2d31] border-b border-black/25">
        ...
        <span class="text-body-sm font-semibold text-[#dbdee1] truncate">
        ...
        {#each chosen as item (item.key)}
          <div class="flex items-center gap-2.5 rounded-md bg-[#2b2d31] px-2.5 py-2">
            ...
            <div class="min-w-0 flex-1">
              <p class="text-body-sm font-medium text-[#f2f3f5] truncate">{item.name}</p>
              <p class="text-2xs text-[#949ba4] truncate">{effectOf(item)}</p>
            </div>
            <span class="shrink-0 text-xs font-semibold tabular-nums text-[#f0b232]">
              {emoji} {priceOf(item).toLocaleString('fr-FR')}
            </span>
          </div>
        {:else}
          <p class="text-xs text-[#949ba4] py-8 text-center">{m.onb_economy_shop_empty()}</p>
        {/each}
      </div>
    </div>
  {/snippet}

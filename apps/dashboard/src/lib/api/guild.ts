...
/**
 * Une etape du pilote automatique du tunnel.
 *
 * `structure` n'est pas ici : elle a sa route (`applyServerTemplate`), avec son
 * verrou de concurrence et sa trace d'audit. Ces trois-la sont ce que la
 * maquette ne fait pas - brancher les reglages sur les salons qu'elle vient de
 * poser, publier ce qui se voit sur Discord, ouvrir l'essai.
 */
export type AutopilotStep = 'wiring' | 'demo' | 'trial';
...
export type AutopilotStepResult = {
  ...
  /** Present sur l'etape `trial` seulement. */
  trial?: {
    granted: boolean;
    days: number;
...
export async function runAutopilotStep(
  ...
  return dashboardRequest('/onboarding/autopilot', {
    method: 'POST',
    payload: { step },
    guildId,
    // La page rend elle-meme chaque etape et son echec : le toast generique du
    // socle doublerait ce qu'elle affiche deja ligne a ligne.
    silent: true,
    errorContext: 'API Error (Autopilot):'
  });
}
